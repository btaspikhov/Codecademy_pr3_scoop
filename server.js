...
function deleteComment(url, request) {
  ...
  if (savedComment && database.comments[id]) {
...
function saveDatabase() {
  ...
  yaml.write('database.yaml', database, err => {if (err) {throw err}
  else console.log('done');});
}

function loadDatabase() {
  const yaml = require('node-yaml')
  yaml.read('database.yaml', {encoding: "utf8", schema: yaml.schema.defaultSafe}, 
  function(err, data) {
    if (err) throw err;
    database = data;
